...
async fn chat(
    ...
    let (fixed_outfit, outfit_items) = if !anchors.is_empty() {
        let result = build_final_outfit(anchors[0], &clothes, user_profile.as_ref(), temperature);
        match result {
    ...
    if !fixed_outfit.is_empty() {
        ...
        let system_prompt = format!(
            r#"너는 아메카지/밀리터리 빈티지/워크웨어 믹스 전문 코디 해설 AI다.

서버가 아래 착장을 최종 결정했다. 너는 이 착장을 변경하지 말고, 왜 이 조합이 자연스러운지 설명만 해라.

확정 착장:
{outfit}
{weather}
규칙:
- 아이템 교체 금지. 새로운 추천 금지. slot 변경 금지.
- 질감, 톤 흐름, 밸런스, 계절감 관점에서 1~3줄로 설명.
- 과장하지 말고 자연스럽게.
- JSON으로만 응답:
{{
  "reason": "설명 1~3줄"
