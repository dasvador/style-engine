...
// 실제 아이템 명칭(M-65)을 그대로 쓰는 픽스처 헬퍼라 snake_case를 적용하지 않는다.
...
fn M65() -> ItemBuilder {
    ItemBuilder {
        ...
        tone: Tone::Mid,
        saturation: Saturation::Mid,
        style: Style::Military,
        weight: Weight::Heavy,
        role: Role::SoftAccent,
        color_temp: "warm",
        statement: 3,
        formality: 2,
    }
...
fn test_bonus_season_mismatch() {
    ...
    println!(
        "Test bonus — score: {}, problems: {:?}",
        result.score,
        problem_codes(&result)
    );
    assert!(
        has_problem(&result, &IssueCode::SeasonalMismatch),
        "시즌 미스매치 미감지. problems: {:?}",
        problem_codes(&result)
    );
}
