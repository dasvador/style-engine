...
기능은 여러 개지만, 구조를 지탱하는 결정은 네 가지입니다.

| | 결정 | 코드 |
|---|---|---|
| 1 | 판정은 규칙 엔진이, 서술만 LLM이 — 비결정성을 한쪽에 가둬 테스트 가능하게 | [`services/style_engine.rs`](src/services/style_engine.rs) |
...
| 지표 | 값 |
|---|---|
| Hard filter 정확도 | **94.8%** (FP 3건 / FN 2건) |
...
### ~~2. `thickness` 어휘 분열~~ → 해결됨

`role`/`tone`/`style` 정리 때 빠져 있던 필드입니다. 같은 2026-05 시드가 이 컬럼에도 다른
어휘를 넣어(`얇은`/`중간`/`두꺼운` 141건 vs 계약값 `thin`/`medium`/`thick` 140건),
온도 게이트가 비교하는 `thin`이 281건 중 **1건**뿐이었습니다.

정규화 후 `thin` 1 → 40건. 그중 **19건은 `weight`가 NULL이라 `thickness`가 게이트를 걸
유일한 근거**였고, 지금까지 한 번도 발동하지 못했습니다.

정규화 방향을 영어로 잡은 이유는 이 필드의 계약이 처음부터 그랬기 때문입니다 — 데이터 모델
문서, 등록 폼의 option value, Vision 프롬프트가 모두 `thin/medium/thick`을 쓰고 UI는 표시할
때만 한국어로 옮깁니다. 서로 다른 값을 요구하던 Vision 프롬프트 두 개도 계약에 맞췄습니다.

### ~~3. Today-fit이 지나치게 관대~~ → 61.5% → 74.0%

오분류 37건을 뜯어보니 임계선 문제가 아니라 원인이 넷으로 갈렸습니다.

| 원인 | 조치 | 효과 |
|---|---|---|
| accessory 격식 gap 을 today_fit 이 보지 않음 | 같은 계층의 `serving_adjustment` 신호를 연결 | +12.5%p |
| 데이트 + 러닝화가 착장 격식에 따라 통과 | 격식과 무관하게 실패로 고정 | +1.0%p |
| 더위 방향 게이트가 아예 없음 | 26도 이상 두꺼운 상하의 → 실패 추가 | |
| 경계 온도(13/18도)가 나쁜 쪽으로 떨어짐 | 경계값은 관대하게, 얇은 원단만 예외 | +3.1%p |

가장 큰 몫은 첫 줄입니다. **필요한 신호는 이미 계산되고 있었는데 today_fit 이 그 값을 보지
않았을 뿐**이었습니다. 임계값은 현재 Pass 판정 케이스들의 패널티 분포에서 정했습니다.

넣었다가 **뺀 것도 있습니다.** "20도 이상 + 무거운 아우터 → 경계" 규칙은 해당 4건의 정답이
Pass 2 / Borderline 1 / Fail 1 로 갈려 신호가 분리되지 않았습니다. 맞히는 1건보다 깨뜨리는
2건이 많아 제거했습니다.

### 4. 케이스 라벨 자체의 모순 — 정확도 상한

튜닝 중에 **입력이 완전히 같은데 정답이 다른 케이스 쌍**을 두 건 발견했습니다. 어떤 엔진도
둘 다 맞출 수 없으므로 달성 가능한 정확도에 상한이 걸립니다. 둘 다 fixture 작성 실수였고
(하나는 "두께만 다른 변형"을 의도했으나 같은 아이템을 참조), 해소한 뒤
[상시 검출 테스트](tests/eval_scorecard.rs)를 넣어 재발을 막았습니다.

해소되지 않은 것도 있습니다. 데이트 + 러닝화 5건의 정답이 **Fail 3 / Borderline 2** 로
갈립니다. 신호가 같으므로 어느 규칙을 써도 최소 2건은 틀립니다. 지금은 다수 쪽을 따릅니다.

이런 종류의 잡음이 남아 있는 한 today-fit 74.0% 가 곧 "엔진이 26% 틀린다"는 뜻은 아닙니다.
...
### 6. eval 이 덮지 못하는 규칙 분기

온도 게이트는 `weight == 가벼움 || thickness == thin` 인데, 케이스 카탈로그에는 시각적 무게가
'중간'이면서 원단만 얇은 상의를 저온에 세우는 케이스가 없습니다. 그래서 fixture 에 thickness 를
넣어도 **thickness 분기는 eval 에서 한 번도 실행되지 않습니다** (넣기 전후 스코어카드 동일).

라벨을 지어내면 규칙으로 규칙을 검증하는 순환이 되므로, 대신 규칙 자체를 직접 검증하는 단위
