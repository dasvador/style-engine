...
- **설명 가능한 AI** — 규칙 엔진이 판단, LLM은 결과를 자연어로 설명 (베이스/포인트 역할 기반)
...
| 화면 | 라우트 | 역할 |
|------|--------|------|
| **홈** | `#home` | 날씨 + CTA(평가/추천) + 추천 카드 + 옷장 요약(베이스/포인트 비율) |
...
| 중심축 부재 | LackOfStructure | -15 | 베이스 없음 + 가벼움 + accent/warm 편중 |
...
| 슬롯 역할 미스매치 | SlotRoleMismatch | -8 | 이너→베이스/연결템, 하의→베이스/구조템 |
| 강한 이너 | StrongInner | -10 | 아우터 있을 때 포인트 이너 |
| 가방 부조화 | BagConflict | -6/-12 | 다른 포인트 있으면 severe |
| 신발 밸런스 | SlotRoleMismatch | -8 | 포인트 신발, 스타일 충돌 |
